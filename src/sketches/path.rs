...
impl Path {
    ...
    ///
    /// ```rust
    /// use anvil::{angle, Circle, IntoLength, Edge, length, Path, point, Rectangle, Plane};
    ///
...
mod tests {
    ...
    use super::*;
    use crate::{IntoLength, angle, dir, length, point};
    ...
    fn end_arc_positive_radius_angle() {
        let path = Path::at(point!(0, 0)).arc_by(length!(1 m), angle!(90 deg));
    ...
    fn end_direction_arc_positive_radius_angle() {
        let path = Path::at(point!(0, 0)).arc_by(length!(1 m), angle!(45 deg));
        assert_dir_eq(path.end_direction(), dir!(1, 1))
    }
